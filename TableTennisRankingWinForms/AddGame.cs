        ...
        private void CreateGameButton_Click(object sender, EventArgs e)
        {
            ...
            if (FilesAccess.db.games.Count(
                    x => (((x.IdPlayerA == playerAId && x.IdPlayerB == playerBId) ||
                        ...
                        x.Date == gameDate.Value)) > 0)
            {
                Console.WriteLine(FilesAccess.db.games.Single(
                    x => ((x.IdPlayerA == playerAId && x.IdPlayerB == playerBId) ||
                        (x.IdPlayerA == playerBId && x.IdPlayerB == playerAId) &&
                        x.Date == gameDate.Value)).Date);
            ...
            FilesAccess.db.games.AddgamesRow(
                playerAId,
                playerBId,
                Convert.ToInt32(setsPlayerA.Text),
                Convert.ToInt32(setsPlayerB.Text),
                IdPlayerWinner,
                gameDate.Value,
                FilesAccess.db.championships.Single(x => x.Name == champCombobox.Text).Id,
                0);

            FilesAccess.db.UpdateGames();

            pointsExchange.Text = playerAName.Text;
        }
    }
}